...
5. En utilisant la fonction `apply()`, calculez pour chaque gène les paramètres suivants, et stockez-les dans une `data.frame()`.  
    ...
    - $s_2, s_2$ écarts-types des échantillons;
    - $\hat{\sigma}_1, \hat{\sigma}_2$ estimations des écarts-types des populations dont sont extraits les échantillons du 1er et 2d groupe, respectivement;
...
7. Ecrivez une boucle `for`qui applique ce même test à chaque ligne de la table d'expression, et récupère dans un tableau les résultats du test, avec une ligne par gène, une colonne par statistique pertinente produite par t.test(). Mesurez le temps d'exécution des 22.000 tests de Student avec cette boucle. 

8. Ecrivez une fonction R dénommée `t.test.wrapper` qui prend en entrée 2 vecteurs (expression et classes de cancer, respectivement), effectue un tst de Student, et retourene un vecteur avec les statistiques pertinentes. 
