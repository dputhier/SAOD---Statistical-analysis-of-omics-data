---
title: 'Exploring Student test with microarray data
...
The goal of this practical is to understand the principles of **Student t-test** by exploring step-by-step the different components of the $t_obs$ statistics. To this purpose, we will apply a test to detect genes differentially expressed in between two cancer types in microarray transcriptomic profiles. 
...
Data were produced using Affymetrix geneChips (Affymetrix Human Genome U133A Array, HGU133A). Information related to this platform are available on GEO website under identifier [GPL96](http://www.ncbi.nlm.nih.gov/projects/geo/query/acc.cgi?acc=GPL96).
...
print(paste("Microarray data folder: ", microarray.data.url))
...
expr.file <- file.path(microarray.data.url, "GSE13425_Norm_Whole.txt") ## Build the full URL of the matrix
message("Downloading matrix table from ", expr.file)
...
We will not load the **phenotypic data**, which is crucial to any analysis since this data type contains the information on all the samples found in a microarray expression matrix. 

```{r load_pheno_and_amp}
## Load phenotypic data
pheno.file <- file.path(microarray.data.url, 'phenoData_GSE13425.tab')
...
## We can also the number of samples per subtype of leukemia in a well-formated table 
kable(as.data.frame(cancer.type.counts),col.names="Samples per type",
...
amp <- read.table(file.path(microarray.data.url, "GSE13425_AMP_Whole.txt"),
                  sep="\t", head=T, row=1)
...
We will now define and create a directory to store the results on our computer. 
...
#### Q1. Count the number of samples per subtype of ALL, choose two groups (g1, g2) with at least 30 individuals, and build two vectors (g1.columns, g2.columns) indicating which columns of the matrix correspond to these respective groups.
