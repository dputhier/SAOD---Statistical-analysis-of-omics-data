...
hist(as.matrix(log2(count.table + 1)), breaks=100, col="blue", border="white")

## Boxplots
boxplot(log2(count.table + 1), col=col.pheno, pch=".", las=1, 
xaxt='n', xlab="Samples", ylab="log2(Counts +1)")
```
...
# Eliminating zero-count genes

All genes from genome the S. cerevisiae where quantified. However only a fraction of them where expressed and some of them where to weakly expressed to be detected in any of the sample. As aresult the count table may contain rows with only zero values (null counts). 
...
col.pheno <- col.pheno[selected.samples]
```
...
boxplot(log2(counts(dds.norm)+1),  col=col.pheno, cex.axis=0.7, las=2)
boxplot(log2(counts(dds.norm, normalized=TRUE)+1),  col=col.pheno,, cex.axis=0.7, las=2) 
plotDensity(log2(counts(dds.norm)+1),  col=col.pheno, cex.lab=0.7) 
plotDensity(log2(counts(dds.norm, normalized=TRUE)+1),  col=col.pheno, cex.lab=0.7)
```
