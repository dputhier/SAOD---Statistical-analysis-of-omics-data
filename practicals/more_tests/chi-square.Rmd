...
### Using chi-squared to test for goodness of fit
...
# from the expected frequencies of fair dice. 
```
</div>
...
- What is the probability of observing $0,1...4,>4$ under a Poisson model with $\lambda=\mu$. Use de *dpois()* function ? 
- What is the expected number of observation in each class under the same Poisson model ?
- Compute the $\chi^2$ statistics (for the vector Y the number of observation greater than 4 is 0 !).
- Compute the p-value for the associated $\chi^2$ statistics. 
- Conclude.
</div>
<div class="hideshow"> << Hide | Show >> </div>
...
## What is the probability of observing 0,1...4,>4 under a Poisson model with lambda=mu
prob <- c(dpois(0:4, lambda = mu), 1-sum(dpois(0:4, lambda = mu))) 
# or
prob <- c(dpois(0:4, lambda = mu), ppois(q=4, lambda = mu, lower.tail = FALSE)) 
...
chi_stat <- sum((c(y,0)-expected)^2/expected)

## Compute the p-value for the associated chi-squared statistics.
pchisq(q=chi_stat, df=length(c(y,0))-2, lower.tail = FALSE)
