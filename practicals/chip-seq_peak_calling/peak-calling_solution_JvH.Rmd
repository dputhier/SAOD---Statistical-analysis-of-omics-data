...
```{r}
#dir.base <- "http://pedagogix-tagc.univ-mrs.fr/courses/ASG1"
dir.base <- "/Users/jvanheld/ASG1" ## Temporary testing on my computer

dir.bedg <- file.path(dir.base, "data/chipseq/Escherichia_coli_FNR_vs_input")

window.size <- 200

test.bedg.file <- paste(sep="", 
                        "FNR_ChIP-seq_Anaerobic_A_GSM1010219_reads_per_",
                        window.size,"bp.bedg")
test.bedg <- read.table(file.path(dir.bedg, test.bedg.file))
names(test.bedg) <- c("chrom", "start", "end","counts")

input.bedg.file <- paste(sep="", "Escherichia_coli_K_12_MG1655_input_reads_per_",window.size,"bp.bedg")
