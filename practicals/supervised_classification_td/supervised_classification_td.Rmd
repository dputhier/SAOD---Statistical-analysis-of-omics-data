...
the input directories (data, **R*</span>** utilities) and creating output
directories on your computer (the corresponding paths are stored in the variables `dir.results` and 
`dir.figures`).

``` {r loading_configuration_file}
...
``` {r two_arbitrary_genes_plot, fig.width=7, fig.height=7, fig.cap="**Comparison of expression values per sample for two arbitrary genes**. Each dot corresponds to one sample, showe color and label denotes the ALL group of the patient. "}
...
``` {r denboer2009_variance_per_gene, fig.width=10, fig.height=5, fig.cap="**Gene-wise distributions of expression variance and standard deviation**. "}
...
``` {r denboer2009_2_maxvar_genes, fig.width=8, fig.height=8, fig.cap="**Expression values for the two genes with the highest variance**. Labels and colors indicate the ALL group of each patient. Note that some groups appear more or less separated (T, Bt, Bo). "}
...
```{r expression_per_group_boxplots, fig.width=12, fig.height=6, fig.cap="**Expression per ALL classes for the two genes with the highest variance.** "}
par(mfrow=c(1,2))
...
``` {r denboer2009_var_vs_welch_Bh_rank_plot, fig.width=12, fig.height=12, fig.cap="**Comparison between global variance and Welch test significance**. The plots highlight the strong difference between the global sorting criterion (variance computed across all samples) and a subtype-specific criterion (Welch test significance for one ALL subtype versus all others). The ranking . "}
par(mfrow=c(1,2))
...
``` {r prcomp_variance, fig.width=7, fig.height=5}
...
``` {r prcomp_biplot, fig.width=8, fig.height=8, eval=FALSE}
