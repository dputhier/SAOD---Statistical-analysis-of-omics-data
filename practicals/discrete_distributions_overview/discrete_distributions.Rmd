...
a. an alignemnt of exactly $25$ nucleotides?
b. an alignemnt of at least $25$ nucleotides?
...
```{r geometric_densiy, echo=FALSE, fig.width=7, fig.height=5, fig.cap="**Geometric distribution.** "}
...
alphabet <- c("A", "C", "G", "T")
n <- 25
m <- 5 # Max accepted mismatches, alignment stops at mismatch m+1
rep <- 100000
read <- sample(alphabet, size=n, replace = TRUE)
seq <- matrix(nrow=rep, ncol=n, sample(alphabet, size=n * rep, replace = TRUE))
...
legend("topright", 
       c(paste("m =", m, "mismatches"),
         paste("rep = ", rep, "random sequences"),
